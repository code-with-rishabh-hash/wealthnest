...
export function Button({ children, variant = 'primary', size = 'md', full, onClick, disabled, style: sx, 'aria-label': ariaLabel, type = 'button', }) {
    const { T } = useTheme();
    const variants = {
    ...
    return (_jsx("button", { type: type, onClick: disabled ? undefined : onClick, disabled: disabled, "aria-label": ariaLabel, "aria-disabled": disabled, style: {
            background: t.bg,
            border: bd,
            borderRadius: 10,
            padding: pd,
