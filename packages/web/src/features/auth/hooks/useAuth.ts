...
const EMPTY_VAULT: Omit<VaultData, 'currency' | 'createdAt'> = {
  investments: [],
  bankAccounts: [],
  transactions: [],
  budgets: [],
...
export function useAuth(onUnlock: (data: VaultData, password: string) => void) {
  ...
  const update = <K extends keyof AuthState>(key: K, value: AuthState[K]) =>
  ...
  useEffect(() => {
    ...
    (async () => {
      const att = await storage.get<number>(STORAGE_KEYS.ATTEMPTS);
      ...
      if (att) update('attempts', att);
      const hash = await storage.get<string>(STORAGE_KEYS.HASH);
  ...
  const doLogin = async () => {
    ...
    clearError();
    try {
      const stored = await storage.get<string>(STORAGE_KEYS.HASH);
      if ((await CRY.hash(state.password)) !== stored) {
        ...
        if (newAtt >= MAX_ATTEMPTS) {
          ...
          update(
            'error',
            '\u{1F512} Wrong password. ' + remaining + ' attempt' + (remaining !== 1 ? 's' : '') + ' remaining before data wipe.',
          );
        }
        update('busy', false);
        return;
      ...
      try {
        const data = await CRY.decrypt<VaultData>(enc as any, state.password);
        await storage.set(STORAGE_KEYS.ATTEMPTS, 0);
      ...
      if (!state.wiped) update('error', 'Decryption failed. Please try again.');
    }
    update('busy', false);
  };
